...
//	private Context host;
			...
			try {
				...
				if (RFCardInterface.isCallBackCalled &&
					...
					RFCardEvent rfCardEvent = new RFCardEvent(-1,
							RFCardInterface.notifyEvent.eventData, RFCardInterface.notifyEvent.eventData.length);
	...
	public boolean searchBegin(){
		...
		result = RFCardInterface.searchBegin(ContactlessInterface.CONTACTLESS_CARD_MODE_AUTO, 1, -1);
	...
	public String attatch(){
		...
		int nResult = RFCardInterface.attach(arryATR);
...
//	}
	public String read(int sectorIndex, int blockIndex){
		...
		int result = RFCardInterface.read(sectorIndex, blockIndex, pDataBuffer, pDataBuffer.length);
		String temp = null;
		if(result > 0){
	...
	public boolean write(int sectorIndex, int blockIndex , byte [] str){
		boolean isSuccess = false;
		byte [] bytes = str;
		int result = RFCardInterface.write(sectorIndex, blockIndex, bytes, bytes.length);
...
//	}
	...
	public String transmit(byte [] apdu){
		...
		int result = RFCardInterface.transmit(apduCommand, apduCommand.length, apduResponse);
		if(result < 0){
			res = null;
		}else{
