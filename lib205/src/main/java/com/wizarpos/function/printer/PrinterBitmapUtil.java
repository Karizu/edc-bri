...
public class PrinterBitmapUtil {
    ...
    public static void printBitmap(Bitmap bm, int bitMarginLeft, int bitMarginTop) {
        ...
        Log.e("aotTag", "printBitmapGSVMSB");
    ...
    private static void printBitmapDV2VMSB(Bitmap bm, int bitMarginLeft, int bitMarginTop) {
        ...
        int jniResult = PrinterInterface.PrinterOpen();
        Log.e("HAL_JNI_JAVA", "PrinterOpen: jniResult = " + jniResult);
        jniResult = PrinterInterface.PrinterBegin();
        Log.e("HAL_JNI_JAVA", "PrinterBegin: jniResult = " + jniResult);
        // // reset the printer
        // PrinterInterface.PrinterWrite(new byte[]{0x1B, 0x40},2);
        // enhance the heating time
        // PrinterInterface.PrinterWrite(new byte[]{0x1B, 0x37,
        // 7,(byte)240,2},5);
        ...
        jniResult = PrinterInterface.PrinterWrite(result, result.length);
        Log.e("HAL_JNI_JAVA", "PrinterWrite: jniResult = " + jniResult);
        // Vector<byte[]> vData=checkBufferLength(result);
        // for(int i=0;i<vData.size();i++){
        // byte[] titles=vData.elementAt(i);
        // int lines=titles.length/WIDTH;
        // PrinterInterface.PrinterWrite(new
        // byte[]{0x12,0x56,(byte)(lines&0xff),(byte)((lines>>8)&0xff)},4 );
        // PrinterInterface.PrinterWrite(titles,titles.length);
        // }

        // restore the heating time
        jniResult = PrinterInterface.PrinterWrite(new byte[]{0x1B, 0x37, 7, (byte) 0xFF, 2}, 5);
        Log.e("HAL_JNI_JAVA", "PrinterWrite: jniResult = " + jniResult);
        ...
        // reset the printer
        jniResult = PrinterInterface.PrinterWrite(new byte[]{0x1B, 0x40}, 2);
        Log.e("HAL_JNI_JAVA", "PrinterWrite: jniResult = " + jniResult);

        jniResult = PrinterInterface.PrinterEnd();
        Log.e("HAL_JNI_JAVA", "PrinterEnd: jniResult = " + jniResult);
        jniResult = PrinterInterface.PrinterClose();
        Log.e("HAL_JNI_JAVA", "PrinterClose: jniResult = " + jniResult);
    }
    ...
    private static void printBitmapDV2VMSBslow(Bitmap bm, int bitMarginLeft, int bitMarginTop) {
        ...
        // PrinterInterface.PrinterWrite(new byte[]{0x1B, 0x40},2);
        // enhance the heating time
        PrinterInterface.PrinterWrite(new byte[]{0x1B, 0x37, 7, (byte) 0xFF, 2}, 5);
        // upgrade the print destiny
        // PrinterInterface.PrinterWrite(new byte[]{0x12, 0x23, 0x00 | (28)},3);
    ...
    public static void printBitmapGSVMSB(Bitmap bm, int bitMarginLeft, int bitMarginTop) {
        ...
        // Log.e("-----","PBU185 "+resultP );
        if (resultP < 0) {
            // PrinterInterface.PrinterClose();
            // Log.e("-----","PBU188 "+resultP );
            return;
        }
        // Log.e("-----","PBU191 "+resultP );
        ...
        PrinterInterface.PrinterWrite(result, result.length);
        //PrinterHelper.getInstance().zouzhi(8);
     ...
     * @return buffer with DC2V_HEAD + image length
     */
    public static byte[] generateBitmapArrayGSV_MSB(Bitmap bm, int bitMarginLeft, int bitMarginTop) {
        byte[] result = null;
        int n = bm.getHeight() + bitMarginTop;
        int offset = GSV_HEAD;
    ...
    /**
     * generate the MSB buffer for bitmap printing ESC * m n1 n2 d1 d2 â€¦ dk 1B
     * 2A m n1 n2 d1 d2 â€¦ dk
     *
     * @param bm            the android's Bitmap data
     * @param bitMarginLeft the left white space in bits.
     * @param bitMarginTop  the top white space in bits.
    ...
    /**
     * generate the MSB buffer for bitmap printing ESC * m n1 n2 d1 d2 â€¦ dk<br/>
     * 1B 2A m n1 n2 d1 d2 â€¦ dk<br/>
     * m =0x00,0x01,0x20, 0x21ã€‚<br/>
     * n1=0-255<br/>
     * n2=0-3<br/>
     * dx=0-255<br/>
     * k = n1+256*n2 (m=0,1)<br/>
     * k = (n1+256*n2)*3 (m=32,33)<br/>
     * mç”¨äºŽé€‰æ‹©ç‚¹å›¾æ–¹å¼�ã€‚<br/>
     * 0x00: é«˜åº¦8ç‚¹ï¼Œæ°´å¹³æ–¹å�‘éœ€æ”¾å¤§ä¸€å€�<br/>
     * 0x01: é«˜åº¦8ç‚¹ï¼Œæ°´å¹³æ–¹å�‘ä¸�éœ€æ”¾å¤§<br/>
     * 0x20: é«˜åº¦24ç‚¹ï¼Œæ°´å¹³æ–¹å�‘éœ€æ”¾å¤§ä¸€å€�<br/>
     * 0x21: é«˜åº¦24ç‚¹ï¼Œæ°´å¹³æ–¹å�‘ä¸�éœ€æ”¾å¤§<br/>
     * ç‚¹å›¾é¡ºåº�è¯·å�‚ç…§è‡ªå®šä¹‰å­—ç¬¦å‘½ä»¤<br/>
     * B0 B3 B6 ... Bx: b7<br/>
     * B1 B4 B7 ... b6<br/>
     * B2 B5 B8 ... b5<br/>
     * b4<br/>
        ...
        while (line < pxHeight) {
            ...
            for (int y = 0; y + line < pxHeight && y < blockHeight; y++) {
                for (int x = 0; x < pxWidth; x++) {
                    ...
                    // æ¯�ä¸ªbyteä¸­ï¼Œä»Žé«˜ä½�å¼€å§‹çš„ç¬¬å‡ ä¸ªç‚¹
